...
def change_res(grid: [DataFrame, GeoDataFrame], level: int = 1) -> [DataFrame, GeoDataFrame]:  # type: ignore
    ...
    if (res + level) > res_upper_limit or (res + level) < res_lower_limit:
        raise ValueError(
            f"Resolution exceeded the allowed boundaries {res_lower_limit} - {res_upper_limit}"
        )
    ...
    if int(level) == -1:
        ...
        grid = (
            grid.groupby("parent")
            .agg({"h3_id": list, **var_operations})
            .reset_index()
            .rename(columns={"h3_id": "child_cells"})
            .rename(columns={"parent": "h3_id"})
        )
        return grid
